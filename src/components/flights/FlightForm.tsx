...
interface FlightFormProps {
  ...
  submitLabel?: string;
  attachedFileName?: string; // file already captured from the parsing step
}
...
export function FlightForm({
  ...
  const [ticketFile, setTicketFile] = useState<File | undefined>();
  ...
  function handleDepAirportBlur() {
    ...
    if (code.length === 3) set('departure_airport_code', code);
  }

  function handleArrAirportBlur() {
    const code = form.arrival_airport_code.toUpperCase().trim();
    if (code.length === 3) set('arrival_airport_code', code);
  }
    ...
    try {
      ...
      if (result) {
        setForm(f => ({
          ...
          departure_date: result.departure_scheduled ? result.departure_scheduled.slice(0, 10) : f.departure_date,
          departure_time: result.departure_scheduled ? result.departure_scheduled.slice(11, 16) : f.departure_time,
          arrival_date: result.arrival_scheduled ? result.arrival_scheduled.slice(0, 10) : f.arrival_date,
          arrival_time: result.arrival_scheduled ? result.arrival_scheduled.slice(11, 16) : f.arrival_time,
        }));
      }
      setLookupDone(true);
    } catch {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      await onSubmit(form, ticketFile);
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-5 pb-8">
      ...
      <div className="flex gap-3">
        <div className="flex-1">
          <Field label="Flight number *">
            <input type="text" className={inputClass} placeholder="e.g. AI101"
              value={form.flight_number} onChange={e => set('flight_number', e.target.value.toUpperCase())} required />
          </Field>
        </div>
        <div className="w-32">
          <Field label="Date *">
            <input type="date" className={inputClass} value={form.departure_date}
      ...
      {import.meta.env.VITE_AVIATIONSTACK_API_KEY && !lookupDone && (
        <Button type="button" variant="secondary" onClick={handleLookup} loading={lookingUp} className="w-full">
          {lookingUp ? 'Looking up flight…' : 'Auto-fill from flight number'}
        </Button>
      )}

      <Field label="Airline">
        <input type="text" className={inputClass} placeholder="e.g. Air India"
      ...
      <div className="bg-slate-800/50 rounded-xl p-4 flex flex-col gap-4 border border-slate-700">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="Airport code *">
            <input type="text" className={inputClass} placeholder="BOM" maxLength={3}
              value={form.departure_airport_code}
              onChange={e => set('departure_airport_code', e.target.value.toUpperCase())}
              onBlur={handleDepAirportBlur} required />
          </Field>
          <Field label="Terminal">
            <input type="text" className={inputClass} placeholder="T2"
              value={form.terminal_departure ?? ''} onChange={e => set('terminal_departure', e.target.value)} />
          </Field>
        </div>
        <Field label="Airport name">
          <input type="text" className={inputClass} placeholder="Chhatrapati Shivaji Maharaj International"
            value={form.departure_airport_name ?? ''} onChange={e => set('departure_airport_name', e.target.value)} />
        </Field>
        <Field label="City">
          <input type="text" className={inputClass} placeholder="Mumbai"
            value={form.departure_city ?? ''} onChange={e => set('departure_city', e.target.value)} />
        </Field>
        <div className="grid grid-cols-2 gap-3">
          <Field label="Date *">
            <input type="date" className={inputClass} value={form.departure_date}
          ...
          <Field label="Local time *">
            <input type="time" className={inputClass} value={form.departure_time}
              onChange={e => set('departure_time', e.target.value)} required />
          </Field>
        </div>
        {form.departure_airport_code.length === 3 && (
          <p className="text-xs text-slate-500">Timezone: {getAirportTimezone(form.departure_airport_code)}</p>
        )}
      </div>
      ...
      <div className="bg-slate-800/50 rounded-xl p-4 flex flex-col gap-4 border border-slate-700">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Field label="Airport code *">
            <input type="text" className={inputClass} placeholder="DXB" maxLength={3}
              value={form.arrival_airport_code}
              onChange={e => set('arrival_airport_code', e.target.value.toUpperCase())}
              onBlur={handleArrAirportBlur} required />
          </Field>
          <Field label="Terminal">
            <input type="text" className={inputClass} placeholder="3"
              value={form.terminal_arrival ?? ''} onChange={e => set('terminal_arrival', e.target.value)} />
          </Field>
        </div>
        <Field label="Airport name">
          <input type="text" className={inputClass} placeholder="Dubai International"
            value={form.arrival_airport_name ?? ''} onChange={e => set('arrival_airport_name', e.target.value)} />
        </Field>
        <Field label="City">
          <input type="text" className={inputClass} placeholder="Dubai"
            value={form.arrival_city ?? ''} onChange={e => set('arrival_city', e.target.value)} />
        </Field>
        <div className="grid grid-cols-2 gap-3">
          <Field label="Date *">
            <input type="date" className={inputClass} value={form.arrival_date}
              onChange={e => set('arrival_date', e.target.value)} required />
          </Field>
          <Field label="Local time *">
            <input type="time" className={inputClass} value={form.arrival_time}
              onChange={e => set('arrival_time', e.target.value)} required />
          </Field>
        </div>
        {form.arrival_airport_code.length === 3 && (
          <p className="text-xs text-slate-500">Timezone: {getAirportTimezone(form.arrival_airport_code)}</p>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <Field label="Gate">
          <input type="text" className={inputClass} placeholder="A12"
            value={form.gate ?? ''} onChange={e => set('gate', e.target.value)} />
        </Field>
        <Field label="Seat">
          <input type="text" className={inputClass} placeholder="23A"
      ...
      <Field label="Booking reference">
        <input type="text" className={inputClass} placeholder="ABC123"
          value={form.booking_reference ?? ''} onChange={e => set('booking_reference', e.target.value.toUpperCase())} />
      </Field>

      <Field label="Price" hint="Free text, e.g. '42,000 INR'">
        <input type="text" className={inputClass} placeholder="42,000 INR"
          value={form.price ?? ''} onChange={e => set('price', e.target.value)} />
      </Field>

      {/* Ticket attachment */}
      <Field label="Ticket (optional)" hint="PDF, JPG, PNG or screenshot">
        {attachedFileName && !ticketFile ? (
          <div className="flex items-center gap-3 bg-emerald-900/30 border border-emerald-700/50 rounded-lg px-3 py-2.5">
            ...
            <span className="text-sm text-emerald-300 flex-1 truncate">{attachedFileName}</span>
            <label className="text-xs text-slate-400 cursor-pointer hover:text-white underline underline-offset-2">
              Change
              <input type="file" accept={ACCEPTED_TICKET_TYPES} className="hidden" onChange={e => setTicketFile(e.target.files?.[0])} />
            </label>
          </div>
        ) : (
          <label className="flex items-center gap-3 bg-slate-800 border border-slate-700 rounded-lg px-3 py-2.5 cursor-pointer hover:border-slate-600">
            ...
            <span className="text-sm text-slate-400 flex-1 truncate">
              {ticketFile ? ticketFile.name : 'Attach ticket'}
            </span>
            <input type="file" accept={ACCEPTED_TICKET_TYPES} className="hidden" onChange={e => setTicketFile(e.target.files?.[0])} />
          </label>
        )}
...
/** Convert FlightFormData → UTC-based DB fields. trip_id is added by the caller. */
export function formDataToFlight(data: FlightFormData) {
  ...
  return {
    ...
    departure_datetime_utc: localToUTC(data.departure_date, data.departure_time, depTz).toISOString(),
    ...
    arrival_datetime_utc: localToUTC(data.arrival_date, data.arrival_time, arrTz).toISOString(),
